...
Currently, there isn't a way to implement Gaussian processes in Julia in a way that support both GPU acceleration and differentiation.
To fill the void, I implemented a very minimal package (or a snippet rather) that fills the void.
...
### Likelihood
The (non-marginal) log density of a Gaussian process function is
\begin{equation}
...
For the gradients, the GPML {% cite rasmussen_gaussian_2006 --file references %} book shows how to differentiate this expression.
...
\end{align}
$$
where, clearly, there is a lot of opportunities for reuse.
...
<div class="center">
{% include figure.html path="assets/img/gp_cuda_scaling.png" class="img-fluid rounded z-depth-1" %}
</div>
